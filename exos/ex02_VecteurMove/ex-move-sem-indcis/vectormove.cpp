 ...
 *  - prendre note de la sortie du programme - observer le comportement de la sémantique de copie.
 *  - implémenter le constructeur de déplacemenent et l'affectation de déplacement afine de compléter la règle des cinq.
 *  - prendre note de la sortie du programme - observer le comportement de la sémantique de déplacement.
 *
 * Note: modifier uniquement la classe Vector, le programme de test en dessous devra rester identique.
 *
*************/
...
public:
    ...
    Vector(const Vector& other)
        ...
        std::cout << name_ << " : copy constructed from: " << other.name_ << std::endl;
        std::copy(other.data_, other.data_ + other.sz_, data_);
    }

    Vector(Vector&& other) : data_{std::exchange(other.data_, nullptr)}, sz_{std::exchange(other.sz_, 0)}, name_{std::move(other.name_)}{
        std::cout << name_ << " constructed from std::move" << std::endl;
    }
    ...
    Vector& operator=(Vector&& other){
        ...
        std::cout << name_ << " : assigned from move " << other.name_ << std::endl;
        delete[] data_;
...
const void printVectorInfo(const Vector& v) {
    ...
    std::cout << "    Memory address of first element: " << std::hex << &v[0] << std::endl;
    std::cout << "    Memory address of vector: " << std::hex << &v << std::endl;
}

Vector createVector() {
    Vector v(5, "VectorFromFun");
    std::cout << "VectorFromFun, crée dans la fonction createVector:" << std::endl;
    printVectorInfo(v);
    return v;
}
...
int main() {
    ...
    std::cout << "v1 - après tentative de déplacement de v1 dans v2:" << std::endl;
    printVectorInfo(v1);
    std::cout << "v2 - après tentative de déplacement de v1 dans v2:" << std::endl;
    printVectorInfo(v2);
    ...
    std::cout << "VectorFromFun, tel que retourné par createVector()" << std::endl;
    printVectorInfo(vectorFromFun);

    // Passage d'un temporaire à process vector
    std::cout << "Passage de VectorFromFun avec tentative de déplacement à processVector" << std::endl;
